...
|---|---------|------|----------|-----------|------------|
| 1 | `retain_leaves_stale_len_on_panic_042b64a_1` | `retain_leaves_stale_len_on_panic` | `compact_str/src/lib.rs:1217` | `marauders` | `042b64a8a2ff225e9485d7676a53a503ba467d6f` |
| 2 | `try_reserve_overflow_silent_ae5f2bc_1` | `try_reserve_overflow_silent` | `compact_str/src/repr/mod.rs:224` | `marauders` | `ae5f2bc02261c7b28b8448e1e26e9b6eaf0191e0` |

## Property Mapping

| Variant | Property | Witness(es) |
|---------|----------|-------------|
| `retain_leaves_stale_len_on_panic_042b64a_1` | `RetainPanicPreservesUtf8` | `witness_retain_panic_preserves_utf8_case_panic_mid_ascii`, `witness_retain_panic_preserves_utf8_case_panic_first_char` |
| `try_reserve_overflow_silent_ae5f2bc_1` | `TryReserveOverflowReturnsErr` | `witness_try_reserve_overflow_returns_err_case_from_nonempty_max`, `witness_try_reserve_overflow_returns_err_case_from_16char_near_max` |
...
### 1. retain_leaves_stale_len_on_panic
...
- **Source**: [#413](https://github.com/ParkMyCar/compact_str/pull/413) — fix: retain not set len if predicate panics (#413)
  > `CompactString::retain` only called `self.set_len(dest_idx)` at the end of the loop. If the user predicate panicked, the unwind bypassed that call and left the string reporting its original length and bytes, diverging from `std::string::String::retain`'s unwind-safety contract. The fix wraps the loop in a `SetLenOnDrop` guard.
- **Fix commit**: `042b64a8a2ff225e9485d7676a53a503ba467d6f` — fix: retain not set len if predicate panics (#413)
...
- **Source**: Check for overflow in `reserve()`
  > `Repr::reserve(additional)` computed `len + additional` with wrapping `usize` arithmetic, so an additive overflow produced a tiny capacity that pretended to fit. The fix replaces the addition with `checked_add` and propagates the overflow as `ReserveError`, letting `try_reserve` return `Err` instead of silently under-allocating.
- **Fix commit**: `ae5f2bc02261c7b28b8448e1e26e9b6eaf0191e0` — Check for overflow in `reserve()`
